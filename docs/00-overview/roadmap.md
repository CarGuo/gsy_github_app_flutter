...
### 2.3 flutter analyze 剩 1 条 warning（原 7 条已收 6 条）
...
- [nested_refresh.dart#L526](file:///Users/guoshuyu/workspace/flutter-work/gsy_github_app_flutter/lib/widget/pull/nested/nested_refresh.dart#L526)：`axisAlignment` deprecated —— **待办**，属运行时行为改动，按 AGENTS 需要真机截图，未合入

### 2.4 CI 已加 flutter test（白名单），仍有一处已知红测试

GitHub Actions 现已在 build job 里加了 `flutter test` 一步（`Run unit / widget tests (whitelist)`），产出报告有了。
...
- `test/page/issue/issue_timeline_merge_test.dart`（新沉淀，issue-timeline-flash 回归契约）

**已知红测试（暂被 CI 白名单绕过，需要跟进）**：

- `test/widget/issue_timeline_item_test.dart` 中 4 个 `reviewed + body` 用例挂在 render 阶段，
  根因是 [_reviewBodyCard](file:///Users/guoshuyu/workspace/flutter-work/gsy_github_app_flutter/lib/page/issue/widget/issue_timeline_item.dart#L222-L246)
  给 `BoxDecoration` 同时设置了非 uniform 的 `Border`（左侧 3px 色条 + 其他 3 边 0.5px 半透）与
  `borderRadius: BorderRadius.circular(6)`，触发 Flutter 断言
  "A borderRadius can only be given on borders with uniform colors"。
  修法：把色条独立成一层 `Container`（宽度 3、纯色），主体保留 uniform border + borderRadius。
  属 UI 档次改动，按 AGENTS 需要真机截图，未合入。修完后把 `issue_timeline_item_test.dart`
  加回 [ci.yml#L48-L54](file:///Users/guoshuyu/workspace/flutter-work/gsy_github_app_flutter/.github/workflows/ci.yml#L48-L54) 白名单。
