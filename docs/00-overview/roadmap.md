  ...
  **进度跟踪**（分阶段推进，避免一口气吞完）：
  ...
  - ⏳ **§3.1 剩余分支（下一子任务）**：
    ...
    2. **answer 徽标细化 + author self-answer + bot 评论徽标 + `[deleted]` 边界 + release-linked footer**
       ...
       - **真机验收缺口（挪到 pt.4 旋转 override 之后）**：`#417` 走 answered + self-answer + bot chip + 嵌套 reply 截图，`#697` 走 deleted body 截图；release-linked footer 卡本轮暂未落 UI（footer 仅是 §3.1 pt.5 的 Discussion body linked-release 特殊卡片，需在 body 段单独识别 `<div class="Layout-sidebar"><a href=".../releases/tag/...">` 结构，暂放到下一 milestone 与真机验收合并做）
