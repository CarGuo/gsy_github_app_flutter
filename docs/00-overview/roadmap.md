...
- **Discussions 阅读页**
  ...
  **进度跟踪**（分阶段推进，避免一口气吞完）：
  ...
  - ⏳ **§3.1 剩余分支（下一子任务）**：
    ...
    - ✅ **2026-09-09 Wave 1 双姿态真机验收整体销账（pt.1 视觉侧 / pt.2 / pt.3 视觉侧 / pt.4 / pt.5）**：本轮跑 debug build + `gsy_pixel_fold` 模拟器（`emulator-5554`, `versionName 8.1.1`），按 [AGENTS.md §折叠 + 非折叠双姿态强制（2026-09-09 拍板）](file:///d:/workspace/project/gsy_github_app_flutter/AGENTS.md) 走 `adb emu fold` / `adb emu unfold` 双姿态硬件切换，每条路径 CLOSED + OPENED 各 1 张截图 + `mcp_dart get_runtime_errors` 各 1 次。证据 / SOP / 缺口三段式见 [tool/ai/smoke/evidence/20260909-discussion-verify/README.md](file:///d:/workspace/project/gsy_github_app_flutter/tool/ai/smoke/evidence/20260909-discussion-verify/README.md) 与 [tool/ai/smoke/open_discussion_verify.md](file:///d:/workspace/project/gsy_github_app_flutter/tool/ai/smoke/open_discussion_verify.md)。
      - **命中总结**：pt.1 reactions bar 7 类 emoji 全渲染（视觉侧）；pt.2 `#417` 一次覆盖 answered badge + self-answer + bot chip 三个证据点；pt.2 `#697` 触发 `Failed to load discussion` + GraphQL 报错 + Retry 兜底；pt.3 `vercel/next.js#49607` 命中 `125 comments`（loadMore 前置阈值）；pt.4 `#511` MiroFish CDN 图 + 微信联系人 CDN 图正常渲染，不再回到蓝色链接文本 fallback；pt.5 `#309` `1 replies` 分组 + `honwel` 嵌套 reply `@dosu` mention 高亮 + `edwardzcn-decade` Answer chip；6 条路径 12 张截图 `get_runtime_errors` 全绿。
      - **不再依赖 `probe_device_rotation.ps1` 前置**：`adb emu fold` / `adb emu unfold` 是 Android SDK platform-tools 内置的 Emulator Console 命令，随 Flutter / Android SDK 升级同步演进，替代了之前"物理设备 + `wm user-rotation lock 0` + accelerometer_rotation 关闭"的坐标 tap 依赖链。旧的 [probe_device_rotation.ps1](file:///d:/workspace/project/gsy_github_app_flutter/tool/ai/smoke/probe_device_rotation.ps1) 仅作物理设备遗留冒烟备份保留，不再作为 Wave 1 前置。
      - **本轮已知缺口（挪 Wave 2 收敛）**：
        - a) pt.1 reactions +1/-1 **写操作**（两姿态）：[_toggleReaction](file:///d:/workspace/project/gsy_github_app_flutter/lib/page/discussion/discussion_detail_page.dart#L996-L1005) 是 `_DiscussionDetailPageState` 私有方法，`vm_service eval` 无法跨 State 边界反射调用；亦无顶层 [gsySmoke*](file:///d:/workspace/project/gsy_github_app_flutter/lib/app.dart) hook。**下一步降级为人肉点击**：CLOSED / OPENED 各点一次 reactions bar 里任一 emoji，观察乐观 UI 跳动 + mutation 落库；若要自动化需在 `lib/app.dart` 新增 `gsySmokeToggleDiscussionReaction(...)` 挂钩，走 Riverpod 或 GlobalKey 路径暴露给 vm_service —— 属新增代码改动，本轮不做。
        - b) pt.3 loadMore 60→90 **触发**（两姿态）：[_loadMore](file:///d:/workspace/project/gsy_github_app_flutter/lib/page/discussion/discussion_detail_page.dart#L147-L200) 同样为私有方法且需滚动到列表尾部才由既有 UI 触发（无 ScrollController 暴露）。首次 loadMore 已由 2026-07-29 物理设备真机验收完成（`vercel/next.js#49607` 30→60 那次），第 2 次 60→90 仍未验；本轮双姿态只覆盖 loadMore **前置视觉**（`125 comments` 阈值命中 + reactions bar + body markdown），loadMore 触发同 pt.1 写操作归为 Wave 2 人肉降级。
        - c) pt.5 长嵌套（20+ replies）压力：本轮 `#309` 只有 1 条 reply，覆盖的是**嵌套渲染代码路径**（`_buildReplyRow` + `discussion_comment_replies_count`）而非分页翻页。真长嵌套需另找 fixture，不阻塞 Wave 1。
